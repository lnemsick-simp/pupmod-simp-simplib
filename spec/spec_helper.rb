...
default_libkv_hiera_config=<<-EOM
---
libkv::options:
  backend: default
  backends:
    default:
      type: file
      id: test
      root_path: "%{libkv_file_root_path}"
EOM
...
# Set fact containing root directory for libkv file store.
# Fact is used by generated fixtures/hieradata/libkv.yaml
def set_libkv_file_root_path(libkv_dir)
  RSpec.configure { |c| c.default_facts['libkv_file_root_path'] = libkv_dir }
end
...
#  c.trusted_server_facts = true
  ...
  c.before(:all) do
    ...
    # configure libkv using local filesystem store
    libkv_yaml_file = File.join(fixture_path, 'hieradata', 'libkv.yaml')
    File.open(libkv_yaml_file, 'w') { |f| f.puts default_libkv_hiera_config }
  end
  ...
  c.before(:each) do
    ...
    # specify test-specific temp dir for libkv file store's root path
    @spec_global_libkv_temp = Dir.mktmpdir('libkv')
    set_libkv_file_root_path(@spec_global_libkv_temp)
  end
  ...
  c.after(:each) do
    ...
    # clean up libkv file store
    FileUtils.remove_entry_secure(@spec_global_libkv_temp)
    @spec_global_libkv_temp = nil
  end
end
