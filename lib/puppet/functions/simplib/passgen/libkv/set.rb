...
Puppet::Functions.create_function(:'simplib::passgen::libkv::set') do
  ...
  #   password with standlone use of `simplib::passgen`.
  #
  ...
  def set(identifier, password, salt, password_options={}, backup=true,
    ...
    if backup
      ...
      unless password_info.empty?
        backup_key = "#{key}.last"
        call_function('libkv::put', backup_key, password_info['value'],
          password_info['metadata'], libkv_options)
      end
    end

    key_info = { 'password' => password, 'salt' => salt }
    metadata = {}
    if password_options.key?('complexity')
      metadata['complexity'] = password_options['complexity']
    end

    if password_options.key?('complex_only')
      metadata['complex_only'] = password_options['complex_only']
    end
