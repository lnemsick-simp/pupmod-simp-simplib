...
Puppet::Functions.create_function(:'simplib::passgen::list') do
  ...
  def list(folder=nil, libkv_options={'app_id' => 'simplib::passgen'})
    subfolder = folder.dup
    subfolder = nil if subfolder.is_a?(String) and subfolder.empty?
    use_libkv = call_function('lookup', 'simplib::passgen::libkv',
      { 'default_value' => false })

    results = nil
    if use_libkv
      results = call_function('simplib::passgen::libkv::list', subfolder, libkv_options)
    else
